    ...
    for i, file in enumerate(files):
        ...
        if optimize:
            ...
            if save_intermediate:
                cv2.imwrite(f'/app/shared-assets/{infer_id}_{i}_t0_{input_file_name}', image)
        ...
        logging.debug(f'brightness {initial_brightness} | {white_balance_darkest} < {brightness} | {(image.shape[0], image.shape[1])} | {infer_id}_{i}_{input_file_name}')
        ...
        if optimize:
            ...
            if white_balance_darkest < 0.086 or brightness < 200:
                ...
                if save_intermediate:
                    cv2.imwrite(f'/app/shared-assets/{infer_id}_{i}_t1_{input_file_name}', image)
                ...
                if binary_dark == 'clahe':
                    ...
                    if save_intermediate:
                        cv2.imwrite(f'/app/shared-assets/{infer_id}_{i}_eqh_{input_file_name}', image)
                    ...
                    image = clahe.apply(image)
                    if save_intermediate:
                        cv2.imwrite(f'/app/shared-assets/{infer_id}_{i}_cl0_{input_file_name}', image)
                    # image = cv2.threshold(image, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
                ...
                if save_intermediate:
                    cv2.imwrite(f'/app/shared-assets/{infer_id}_{i}_t2_{input_file_name}', image)
                ...
                if binary_dark == 'clahe+threshold' or binary_dark == 'equalhist':
                    ...
                    if save_intermediate:
                        cv2.imwrite(f'/app/shared-assets/{infer_id}_{i}_dil_{input_file_name}', dil_image)
                        cv2.imwrite(f'/app/shared-assets/{infer_id}_{i}_er_{input_file_name}', er_image)
                ...
                # image = cv2.threshold(image, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
                # if save_intermediate:
                #     cv2.imwrite(f'/app/shared-assets/{infer_id}_{i}_eqhtr_{input_file_name}', image)
